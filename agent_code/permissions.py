...
class PermissionDecision:
    """权限引擎的决策结果，behavior是allow/ask/deny之一"""
...
def _is_dangerous(command: str) -> str | None:
    """检查命令是否匹配危险模式，返回None表示安全，返回字符串表示危险原因"""
...
def decide_permission(request: PermissionRequest) -> PermissionDecision:
    ...
    # plan 模式：只允许只读工具；ask_user_question /web_* 仍会走ask
    # 写类工具一律 deny
    if mode == "plan":
    ...
    # accept Edits 模式： 文件编辑跳过确认 UI，但安全检验仍在 agent.py 做
    if mode == "acceptEdits":
