...
from .memory_types import MemoryEntry, MEMORY_TYPES, make_slug
...
def write_memory(cwd: Path, mem_type: str, title: str, body: str) -> MemoryEntry:
    ...
    topic 文件已经写入，仍能被 recall 时的 scan 找到，不会丢以及。
    """
    ...
    counter = 1
    while tpath.exists():
        tpath = topic_path(cwd, mem_type, f"{slug}_{counter}")
        counter += 1
    ...
    # 生成一句 hook：取 body 的前60个字符，截到最后一个完整词
    hook = body.strip()[:60]
...
def recall_memory(cwd: Path, query: str, top_k: int = 5) -> list[MemoryEntry]:
    ...
    # (score,mtime,entry)三元组：score 高的优先，同分按照mtime新优先
